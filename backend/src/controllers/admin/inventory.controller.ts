// src/controllers/admin/inventory.controller.ts
...
export const updateStock = [
  ...
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      // productId is a validated param (via updateStockValidation)
      const productId = String(req.params.id);

      // body.stock is validated to be int; variantId optional
      const stock = Number(req.body.stock);
      const variantIdStr = req.body.variantId ? String(req.body.variantId) : undefined;
      const variantObjectId = toObjectIdOrUndefined(variantIdStr);
      ...
      try {
        ...
        if (variantObjectId) {
          // product.variants có thể undefined -> dùng fallback []
          const variants = product.variants ?? [];
          const v = variants.find((x: any) => x._id && x._id.equals(variantObjectId));
          if (!v) {
            await session.abortTransaction();
            session.endSession();
          ...
          emitStockChange(productId, { variantId: variantIdStr, stock: v.stock, productStock: product.stock });
        } else {
