...
export const createProductOld = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    const { name, brand, type, category, isNewProduct, price, stock, description, specifications, commitments, condition, sellPrice } = req.body;

    const images = req.files ? (req.files as Express.Multer.File[]).map((file) => `/uploads/product/${file.filename}`) : [];
    ...
    const product = new Product({
      ...
      variants: mapSellPriceToVariants(sellPrice),
      owner: req.user?._id,
      sold: 0,
      isApproved: true,
    });
