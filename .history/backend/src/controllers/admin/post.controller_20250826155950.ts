...
function withAbsolutePostUrls(req: Request, post: any) {
  if (!post) return post;
  ...
  if (Array.isArray(obj.images)) {
    obj.images = obj.images.map((img: string) =>
...
export const createPost = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const post = await Post.create({
      ...
      content,
      images: imagePaths,
      author: user._id, // Thêm trường author
    });

    res.status(201).json({ success: true, post });
  } catch (err: any) {
    console.error('Lỗi tạo bài viết:', err); // Cải thiện log lỗi
