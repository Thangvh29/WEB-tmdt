...
// Create used product (C2C)
export const createUsedProduct = [
  ...productValidation,
  async (req: Request, res: Response, next: NextFunction) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) return res.status(400).json({ errors: errors.array() });
    ...
    try {
      ...
      const productData: any = {
        ...
        description: body.description,
        isNewProduct,
        owner: toObjectId(body.owner),
        category: body.category ? toObjectId(body.category) : undefined,
        condition: body.condition, // bắt buộc phải có nếu isNewProduct=false
      };
      ...
      console.error('Create used product error:', err);
      if (err.name === 'ValidationError') {
        return res.status(400).json({ message: 'Dữ liệu không hợp lệ', details: mongooseValidationErrors(err) });
      }
      next(err);
