...
export const createPost = async (req: Request, res: Response) => {
  try {
    const newPost = new Post({
      ...
      images: req.files ? (req.files as Express.Multer.File[]).map((f) => "/uploads/posts/" + f.filename) : [],
    });

    const savedPost = await newPost.save();

    // Populate author để trả ra giống getFeed
    const populatedPost = await savedPost.populate("author", "name avatar role");

    // Map absolute URL cho images
    const finalPost = {
      ...populatedPost.toObject(),
      images: populatedPost.images.map((img: string) =>
