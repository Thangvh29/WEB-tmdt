...
export const createProductOld = async (req: Request, res: Response, next: NextFunction) => {
  // KHÔNG dùng productValidation vì ảnh lấy từ multer và luật C2C riêng
  try {
    // Lấy body
    const {
      name,
    ...
    // Ảnh từ multer (ưu tiên), nếu muốn gộp thêm ảnh URL từ body.images thì có thể merge bên dưới
    ...
    // Chủ sở hữu
    const resolvedOwner = req.user?._id || ownerFromBody;
    if (!resolvedOwner || !Types.ObjectId.isValid(resolvedOwner)) {
      return res.status(400).json({ message: 'Chủ sở hữu (owner) là bắt buộc và phải hợp lệ cho sản phẩm C2C' });
    }

    // Tình trạng bắt buộc và KHÔNG được "new" cho hàng C2C
    if (!condition) {
      return res.status(400).json({ message: 'Trạng thái (condition) là bắt buộc cho sản phẩm C2C' });
    }
    if (!['like_new', 'good', 'fair', 'poor'].includes(String(condition))) {
      return res.status(400).json({ message: 'Tình trạng cho C2C phải là like_new, good, fair hoặc poor' });
    }

    // Map sellPrice -> variants
    const variants = mapSellPriceToVariants(sellPrice);

    // Làm sạch / kiểm tra biến thể nếu có
    if (variants.length > 0) {
      // so sánh giá gốc/giá bán & chuẩn hoá
      for (const v of variants) {
        if (v.price < 0) return res.status(400).json({ message: 'Giá biến thể không được âm' });
        if ((v as any).compareAtPrice != null && Number(v.compareAtPrice) < Number(v.price)) {
          return res.status(400).json({ message: 'compareAtPrice của biến thể không được nhỏ hơn price' });
        }
        if (v.stock < 0) return res.status(400).json({ message: 'Tồn kho biến thể không được âm' });
        v.images = Array.isArray(v.images) ? v.images.slice(0, 10) : [];
      }

      // chỉ 1 biến thể default
      const defaultCount = variants.filter((v) => v.isDefault).length;
      if (defaultCount > 1) {
        return res.status(400).json({ message: 'Chỉ một biến thể được đặt là mặc định' });
      }
      if (defaultCount === 0) variants[0].isDefault = true;

      // chặn trùng tổ hợp attributes trong chính request
      ...
      for (const v of variants) {
        const key = (v.attributes || [])
          ...
          .join('|');
        if (seen.has(key)) {
          return res.status(400).json({ message: 'Có biến thể trùng tổ hợp attributes trong danh sách gửi lên' });
        }
        seen.add(key);
      }
    } else {
      // Không có variants -> yêu cầu price & stock ở cấp product
      const p = Number(price);
      const s = Number(stock);
      if (!Number.isFinite(p) || p < 0) {
        return res.status(400).json({ message: 'Giá sản phẩm là bắt buộc và không được âm khi không có biến thể' });
      }
      if (!Number.isFinite(s) || s < 0) {
        return res.status(400).json({ message: 'Tồn kho là bắt buộc và không được âm khi không có biến thể' });
      }
    }

    // Dữ liệu để tạo Product (C2C luôn isNewProduct = false)
    const productData: any = {
      name: String(name).trim(),
      brand: String(brand).toLowerCase().trim(),
      type: String(type).toLowerCase().trim(),
      images,
      specs: Array.isArray(specifications) ? specifications : [],
      commitments: Array.isArray(commitments) ? commitments : [],
      description: description ? String(description).trim() : undefined,
      isNewProduct: false,
      owner: toObjectId(String(resolvedOwner)),
      category: toObjectId(String(category)),
      condition: String(condition),
      variants: variants.length > 0 ? variants : [],
      // nếu không có variants, set price/stock ở cấp product
      ...(variants.length === 0
        ? {
        ...
        : {}),
      // Để model tự set isApproved=false cho C2C (pre-save), nhưng có thể set tường minh:
      isApproved: false,
      sold: 0,
    };

    const product = await Product.create(productData);
    return res.status(201).json({ message: 'Thêm sản phẩm C2C thành công', product });
  } catch (err: any) {
