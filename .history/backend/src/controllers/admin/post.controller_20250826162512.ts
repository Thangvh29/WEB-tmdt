...
export const createPost = async (req: AuthRequest, res: Response) => {
  try {
    ...
    // ✅ FIX: Sử dụng đường dẫn đúng với multer config (post không có 's')
    const imagePaths = files ? files.map((file) => `/uploads/post/${file.filename}`) : [];
    ...
    res.status(500).json({ success: false, message: 'Lỗi khi tạo bài viết', error: err.message });
  }
};
  ...
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      if (files && files.length > 0) {
        // ✅ FIX: Sử dụng đường dẫn đúng với multer config
        const newImagePaths = files.map((file) => `/uploads/post/${file.filename}`);
