...
export const getDashboardStats = [
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      // --- KPI 3: Top products by quantity sold ---
      ...
      // --- KPI 4: New users (non-admin) in period ---
      const userFilter: any = { role: { $ne: 'admin' } };
      if (start || end) userFilter.createdAt = matchDate;
      const newUserCount = await User.countDocuments(userFilter) || 0;

      // --- KPI 5: Active users (who made orders) ---
      ...
      // --- KPI 6: Posts & Comments counts (approved) ---
      ...
      const postCount = await Post.countDocuments(postFilter) || 0;
      const commentCount = await Comment.countDocuments(commentFilter) || 0;

      // --- KPI 7: Conversion estimate ---
      const cartFilter: any = {};
      if (start || end) cartFilter.createdAt = matchDate;
      const cartsInPeriod = await Cart.countDocuments(cartFilter) || 0;
      ...
      // Response shape với giá trị mặc định
      return res.json({
        stats: {
          totalRevenue,
          orderCount,
          avgOrderValue,
          statusMap,
          topProducts: topProducts || [], // Mảng rỗng nếu không có sản phẩm
          newUserCount,
          activeUsers,
          postCount,
          commentCount,
          ...
          revenueByDate: revenueByDate.map((r) => ({ date: r._id, total: r.total, count: r.count })) || [], // Mảng rỗng nếu không có dữ liệu
          paymentBreakdown: paymentBreakdown || [], // Mảng rỗng nếu không có dữ liệu
          categoryAgg: categoryAgg || [], // Mảng rỗng nếu không có dữ liệu
        },
      });
    } catch (err) {
