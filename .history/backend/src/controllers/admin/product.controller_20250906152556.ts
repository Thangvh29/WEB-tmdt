    ...
    try {
      ...
      if (variants.length > 0) {
        ...
        productData.stock = variants.reduce(
          (sum: number, v: IVariant) => sum + (v.stock ?? 0),
          0
        );
        productData.price =
          Math.min(...variants.map((v: IVariant) => v.price ?? Infinity)) || 0;
      } else {
        // Nếu không có variants, dùng price/stock từ body
        productData.price =
          body.price !== undefined ? Number(body.price) : 0;
        productData.stock =
      ...
      return res
        .status(201)
        .json({ message: "Thêm sản phẩm cũ thành công", product });
    } catch (err: any) {
      console.error("Create product old error:", err);
      if (err.name === "ValidationError") {
        return res.status(400).json({
          message: "Dữ liệu không hợp lệ",
          details: mongooseValidationErrors(err),
        });
      }
      next(err);
    }
...
export const getOldProducts = [
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      const q = req.query as any;
      const filter: any = { isNewProduct: false };
