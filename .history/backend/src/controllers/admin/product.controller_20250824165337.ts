...
unction toObjectId(id?: string) {
  try {
    return id ? new Types.ObjectId(String(id)) : undefined;
  } catch {
    return undefined;
  }
}
...
export const createProductOld = async (req: Request, res: Response, next: NextFunction) => {
  // Lấy lỗi từ express-validator (nếu bạn đã gắn validation trước router)
  const errors = validationResult(req);
  if (!errors.isEmpty()) return res.status(400).json({ errors: errors.array() });
  ...
  try {
    ...
    // 1) Ảnh: gộp file upload (multer) + body.images (URL)
    const uploadImages = Array.isArray(req.files)
      ? (req.files as Express.Multer.File[]).map(f => `/uploads/product/${f.filename}`)
      : [];
    const bodyImages = Array.isArray(b.images) ? b.images : [];
    const images: string[] = [...uploadImages, ...bodyImages].slice(0, 10);

    if (!Array.isArray(images) || images.length < 3 || images.length > 10) {
      return res.status(400).json({ message: 'Sản phẩm phải có từ 3 đến 10 ảnh' });
    }

    // 2) Kiểm tra trường bắt buộc cơ bản
    if (!b.name || String(b.name).trim().length < 3) {
      return res.status(400).json({ message: 'Tên sản phẩm phải từ 3 ký tự trở lên' });
    }
    if (!b.brand || String(b.brand).trim().length < 2) {
      return res.status(400).json({ message: 'Hãng là bắt buộc (>= 2 ký tự)' });
    }
    if (!b.type) {
      return res.status(400).json({ message: 'Loại sản phẩm là bắt buộc' });
    }
    if (!b.category || !Types.ObjectId.isValid(String(b.category))) {
      return res.status(400).json({ message: 'Danh mục (category) là bắt buộc và phải hợp lệ' });
    }

    // 3) Condition: bắt buộc cho hàng đã qua sử dụng, không cho 'new'
    const cond = String(b.condition || '').trim();
    if (!['like_new', 'good', 'fair', 'poor'].includes(cond)) {
      return res.status(400).json({ message: 'Tình trạng phải là one of: like_new, good, fair, poor' });
    }

    // 4) Owner: admin có thể gán owner từ body, nếu không có dùng req.user._id
    const ownerFromBody = b.owner;
    const ownerIdRaw = (req.user as any)?._id ?? ownerFromBody;
    if (!ownerIdRaw || !Types.ObjectId.isValid(String(ownerIdRaw))) {
      return res.status(400).json({ message: 'Chủ sở hữu (owner) là bắt buộc và phải hợp lệ' });
    }
    const ownerId = new Types.ObjectId(String(ownerIdRaw));

    // 5) Map sellPrice -> variants
    const variants: IVariant[] = Array.isArray(b.sellPrice) ? mapSellPriceToVariants(b.sellPrice) : [];

    // 6) Validate variants OR product-level price/stock
    if (variants.length > 0) {
      let defaultCount = 0;
      for (const v of variants) {
        if (!Number.isFinite(Number(v.price)) || Number(v.price) < 0) {
          return res.status(400).json({ message: 'Giá biến thể không được âm và phải hợp lệ' });
        }
        if (v.compareAtPrice != null && Number(v.compareAtPrice) < Number(v.price)) {
          return res.status(400).json({ message: 'compareAtPrice không được nhỏ hơn price' });
        }
        if (!Number.isFinite(Number(v.stock)) || Number(v.stock) < 0) {
          return res.status(400).json({ message: 'Tồn kho biến thể không được âm và phải hợp lệ' });
        }
        if (v.images && v.images.length > 10) v.images = v.images.slice(0, 10);
        if (v.isDefault) defaultCount++;
      }

      if (defaultCount > 1) return res.status(400).json({ message: 'Chỉ một biến thể được đặt là mặc định' });

      // --- Fix TS2375: mutate trực tiếp thay vì gán object literal ---
      if (defaultCount === 0 && variants.length > 0) {
        variants[0].isDefault = true; // safe vì đã check length > 0
      }

      // chống trùng tổ hợp attributes trong request
      const seen = new Set<string>();
      for (const v of variants) {
        const key = (v.attributes || []).map((a: any) => `${a.name}:${a.value}`).sort().join('|');
        if (seen.has(key)) return res.status(400).json({ message: 'Có biến thể trùng tổ hợp attributes' });
        seen.add(key);
      }
    } else {
      // không có variants -> cần price & stock ở cấp product
      const p = Number(b.price);
      const s = Number(b.stock);
      if (!Number.isFinite(p) || p < 0) return res.status(400).json({ message: 'price là bắt buộc và >= 0 khi không có biến thể' });
      if (!Number.isFinite(s) || s < 0) return res.status(400).json({ message: 'stock là bắt buộc và >= 0 khi không có biến thể' });
    }

    // 7) Build productData (admin thêm used => isNewProduct=false, isApproved=true)
    const productData: any = {
      name: String(b.name).trim(),
      brand: String(b.brand).toLowerCase().trim(),
      type: String(b.type).toLowerCase().trim(),
      images,
      specs: Array.isArray(b.specifications) ? b.specifications : [],
      commitments: Array.isArray(b.commitments) ? b.commitments : [],
      description: b.description ? String(b.description).trim() : undefined,

      isNewProduct: false,
      isApproved: true,     // admin thêm => duyệt luôn
      owner: ownerId,
      category: new Types.ObjectId(String(b.category)),
      condition: cond,
      variants: variants.length > 0 ? variants : [],
      ...(variants.length === 0 ? { price: Number(b.price ?? 0), stock: Number(b.stock ?? 0) } : {}),
      sold: 0,
    };

    const created = await Product.create(productData);
    return res.status(201).json({ message: 'Thêm sản phẩm đã qua sử dụng (admin) thành công', product: created });
  } catch (err: any) {
    console.error('CreateProductOld (admin) error:', err);
    if (err?.name === 'ValidationError') {
      return res.status(400).json({ message: 'Dữ liệu không hợp lệ', details: mongooseValidationErrors(err) });
    }
    return next(err);
