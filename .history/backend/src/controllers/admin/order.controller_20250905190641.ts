...
export const getOrders = [
  ...
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      // Nếu không phải admin/staff thì chỉ được xem đơn của chính mình
      ...
      // ✅ Trả về nguyên gốc items để admin xem chi tiết
      const mapped = (orders as any[]).map(o => ({
        _id: o._id,
        user: o.user,
        items: o.items,          // giữ nguyên danh sách sản phẩm
