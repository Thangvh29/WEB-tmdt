...
export const createProductOld = async (req: Request, res: Response, next: NextFunction) => {
  // Dùng các validator đã khai báo trước đó (nếu có). Ở đây vẫn check errors.
  const errors = validationResult(req);
  if (!errors.isEmpty()) return res.status(400).json({ errors: errors.array() });
  ...
  try {
    ...
    // 1. Ảnh: gộp upload + body.images (ưu tiên file upload)
    ...
    if (variants.length > 0) {
      ...
      for (const v of variants) {
        if (Number(v.price) < 0) return res.status(400).json({ message: 'Giá biến thể không được âm' });
        ...
        if (Number(v.stock) < 0) return res.status(400).json({ message: 'Tồn kho biến thể không được âm' });
      ...
      if (defaultCount === 0 && variants.length > 0) {
        // đặt biến thể đầu tiên làm default (an toàn cho TS)
      ...
      for (const v of variants) {
        const key = (v.attributes || []).map(a => `${a.name}:${a.value}`).sort().join('|');
        if (seen.has(key)) return res.status(400).json({ message: 'Có biến thể trùng tổ hợp attributes' });
        seen.add(key);
      }
    } else {
      // không có variants -> cần price & stock
    ...
    // 7. Build product object (admin thêm used => isNewProduct=false, isApproved=true)
    const productData: any = {
