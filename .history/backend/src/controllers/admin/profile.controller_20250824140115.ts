...
// Helper để build avatar URL tuyệt đối
...
export const getProfile = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (user.role !== 'admin') {
      return res
        .status(403)
        .json({ message: 'Chỉ admin có thể xem profile này' });
    }
...
export const updateProfile = [
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    try {
      ...
      console.log(
        '🔄 Update admin profile - req.file:',
        req.file ? 'Has file' : 'No file'
      );
      ...
      if (user.role !== 'admin') {
        return res
          .status(403)
          .json({ message: 'Chỉ admin có thể cập nhật profile này' });
      }
      ...
      res
        .status(500)
        .json({ message: 'Lỗi server khi cập nhật profile admin' });
    }
  },
];
