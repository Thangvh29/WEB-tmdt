...
export const getInventoryValidation = [
  ...
  query('stockStatus')
    ...
    .withMessage('Trạng thái tồn kho phải là inStock, outOfStock hoặc rỗng'),
...
export const getInventory = [
  ...
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      ...
      const { name, brand, type, stockStatus, page = 1, limit = 10 } = req.query as any;
      const filter: any = { isApproved: true };
      
      if (name) filter.name = { $regex: name, $options: 'i' };
      if (brand) filter.brand = { $regex: brand, $options: 'i' };
      if (type) filter.type = type;
      if (stockStatus) filter.stock = stockStatus === 'inStock' ? { $gt: 0 } : { $eq: 0 };
    ...
    try {
      ...
      try {
        ...
        if (variantId) {
          ...
          const variant = variants.find((v: any) => v._id?.equals(variantId));
          if (!variant) {
            await session.abortTransaction();
            session.endSession();
