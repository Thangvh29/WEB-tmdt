  ...
  const getConversations = [
    ...
    async (req: AuthRequest, res: Response, next: NextFunction) => {
      ...
      try {
        ...
        // coerce query param safely to boolean
        const unreadOnly = String(req.query.unreadOnly) === 'true';

        const filter: any = { participants: { $in: [new Types.ObjectId(String((user as any)._id ?? user._id))] } };

        if (unreadOnly) {
          const convIdsWithUnread = await Message.distinct('conversation', {
            readBy: { $ne: new Types.ObjectId(String((user as any)._id ?? user._id)) },
            deletedFor: { $ne: new Types.ObjectId(String((user as any)._id ?? user._id)) },
          }).exec();
        ...
        const mapped = (convs || []).map((conv: any) => {
          // conv.lastMessage may be an ObjectId or populated object; treat it as any
          ...
          const lastMessage =
            lm && typeof lm === 'object'
              ? {
                  ...
                  isRead: Array.isArray(lm.readBy) ? lm.readBy.some((id: any) => String(id) === String((user as any)._id ?? user._id)) : false,
                }
              : null;
  ...
  const getMessages = [
    ...
    async (req: AuthRequest, res: Response, next: NextFunction) => {
      ...
      try {
        ...
        const q: any = { conversation: conv._id, deletedFor: { $ne: new Types.ObjectId(String((user as any)._id ?? user._id)) } };
        if (beforeId) {
        ...
        const total = await Message.countDocuments({ conversation: conv._id, deletedFor: { $ne: new Types.ObjectId(String((user as any)._id ?? user._id)) } }).exec();
        ...
        const ordered = (msgs || []).reverse().map((m: any) => ({
          ...
          meta: m.meta,
          sender: m.sender,
          isRead: Array.isArray(m.readBy) ? m.readBy.some((id: any) => String(id) === String((user as any)._id ?? user._id)) : false,
  ...
  const sendMessage = [
    ...
    async (req: AuthRequest, res: Response, next: NextFunction) => {
      ...
      try {
        ...
        const populated: any = msg; // as any to avoid strict typing problems
