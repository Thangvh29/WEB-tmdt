...
export const deleteVariant = [
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      const variantIndex = product.variants.findIndex(v => v._id?.equals(req.params.variantId));
      if (variantIndex === -1) return res.status(404).json({ message: 'Không tìm thấy biến thể' });
