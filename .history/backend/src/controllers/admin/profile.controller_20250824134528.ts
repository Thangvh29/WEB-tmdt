...
import config from '../../config/config.js'; // Thêm import config
  ...
  try {
    ...
    res.json({
      ...
      profile: {
        _id: user._id,
        name: user.name,
        avatar: user.avatar ? `${config.BACKEND_URL || 'http://localhost:4000'}${user.avatar}` : '',
    ...
    try {
      ...
      res.json({
        ...
        profile: {
          _id: user._id,
          name: user.name,
          avatar: user.avatar ? `${config.BACKEND_URL || 'http://localhost:5000'}${user.avatar}` : '',
          address: user.address || '',
          phone: user.phone || '',
          email: user.email,
          role: user.role,
