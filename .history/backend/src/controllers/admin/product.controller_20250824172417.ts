...
// ID cố định đại diện cửa hàng/admin
...
export const createProductOld = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    // Lấy ảnh upload
    const images = req.files?.map((file: any) => `/uploads/product/${file.filename}`) || [];
    ...
    const product = new Product({
      ...
      commitments,
      images,
      isOld: true,         // đánh dấu sản phẩm cũ
      isNewProduct: false, // không phải sản phẩm mới
      stock: variants.reduce((sum: number, v: any) => sum + (v.stock || 0), 0) || 0,
    });

    await product.save();

    return res.status(201).json({
      message: 'Tạo sản phẩm cũ thành công',
      product,
    });
  } catch (err) {
    console.error('createProductOld error:', err);
    next(err);
  }
};
