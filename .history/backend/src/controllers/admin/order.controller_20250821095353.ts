...
// Thêm vào controllers/admin/order.controller.ts (hoặc controllers/order.controller.ts)
import { Request, Response, NextFunction } from 'express';
import { Order } from '../../models/order.model.js'; // đường dẫn tương tự phần còn lại của file
...
export const getOrderDetail = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    // Nếu bạn muốn bảo mật: chỉ owner hoặc admin/staff mới xem chi tiết
    // (nếu dùng AuthRequest, bạn có req.user; nếu không, route đã bảo vệ bằng middleware)
    // Example (uncomment nếu muốn kiểm tra):
    // const user = (req as any).user;
    // if (user && user.role !== 'admin' && String(order.user._id || order.user) !== String(user._id)) {
