...
const coreListProductsHandler: RequestHandler = async (req, res, next) => {
  try {
    ...
    // ✅ Phân biệt sản phẩm mới / cũ
    if ((req as any).forceNew) {
      filter.isNewProduct = { $eq: true };
    } else if ((req as any).forceOld) {
      filter.isNewProduct = { $eq: false };
    } else if (isNewRaw !== undefined) {
      const isNewStr = String(isNewRaw).toLowerCase();
      if (isNewStr === "true") filter.isNewProduct = { $eq: true };
      else if (isNewStr === "false") filter.isNewProduct = { $eq: false };
    }
    ...
    const mapped = (products || []).map((p: any) => {
      ...
      return {
        ...
        priceRange,
        priceDisplay:
          priceRange.min === priceRange.max ? priceRange.min : priceRange.min,
        stock: p.stock ?? 0,
        inStock,
...
export const listNewProducts = [
  (req: Request, res: Response, next: NextFunction) => {
    (req as any).forceNew = true;
    return coreListProductsHandler(req, res, next);
  },
];
