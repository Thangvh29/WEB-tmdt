...
export const listOrders = [
  ...
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      // If admin/staff optionally allow querying other users via query.userId (but this is user routes — restrict)
      // By default only return current user's orders
      const user = req.user;
      if (!user) return res.status(401).json({ message: 'Unauthorized' });

      // If the protect middleware sets a minimal object for non-user roles, allow admin/staff to pass ?userId
      const isPrivileged = (user as any).role && (['admin', 'staff'].includes((user as any).role));
      if (isPrivileged && req.query.userId) {
        // allow staff/admin to view specific user's orders on this endpoint only if needed
        filter.user = Types.ObjectId(String(req.query.userId));
      } else {
      ...
      // map a lightweight preview (first item, totals)
      const mapped = orders.map((o: any) => ({
        _id: o._id,
        totalAmount: o.totalAmount,
        status: o.status,
        statusHistory: o.statusHistory?.length ? o.statusHistory[o.statusHistory.length - 1] : null,
