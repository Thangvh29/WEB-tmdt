...
export const getHomeFeed = [
  ...
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      // if user available, mark liked
      const userId = (req.user as any)?._id?.toString();

      const enriched = posts.map((p: any) => ({
        ...p,
        likedByMe: userId ? (p.likes || []).some((id: any) => String(id) === String(userId)) : false,
...
export const commentOnPost = [
  ...
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      const rating: number | undefined = req.body.rating ?? undefined;
      ...
      // If rating provided, ensure post is associated with a product? (we allow ratings only if linked to product)
      // Here we accept rating only if post.product exists; adjust per business rule
      if (rating !== undefined && !post.product) {
        return res.status(400).json({ message: 'Rating chỉ áp dụng cho bài viết liên quan tới sản phẩm' });
      }

      // if parent provided, ensure parent exists and belongs to same post (or is top-level comment)
      let parentRef: Types.ObjectId | undefined;
      if (parentId) {
        ...
        // optional: check parent.post === postId (if parent references post)
        parentRef = parent._id;
      }

      const newComment = await Comment.create({
        content,
        author: (user as any)._id ?? user._id,
        post: post._id,
        product: null,
        parent: parentRef ?? null,
        rating: rating ?? null,
        isApproved: true, // adjust moderation policy if needed
      });
...
export const getPostComments = [
  ...
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      // only non-deleted & approved comments (for public view)
      ...
      // fetch replies count / optionally first-level replies
      const topIds = comments.map((c: any) => c._id);
      const replies = await Comment.find({ parent: { $in: topIds }, isDeleted: { $ne: true }, isApproved: true })
        .sort({ createdAt: 1 })
        .populate('author', 'name avatar')
        .lean()
        .exec();
