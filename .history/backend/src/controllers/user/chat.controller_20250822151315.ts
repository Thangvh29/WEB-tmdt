...
export function createUserChatController(io: IOServer) {
  ...
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      if (!conv.participants.some((p: any) => p.toString() === userIdStr)) {
        return res.status(403).json({ message: 'Bạn không có quyền truy cập tin nhắn này' });
      }

      if (!Array.isArray(msg.readBy)) msg.readBy = [];
      if (!msg.readBy.some((id: any) => id.toString() === userIdStr)) {
        msg.readBy.push(user._id);
        await msg.save();
        emitMessageRead(convId, { messageId: msg._id.toString(), readerId: userIdStr });
      }

      return res.json({ message: 'Đã đánh dấu đã đọc', messageId: msg._id.toString() });
    } catch (err) {
