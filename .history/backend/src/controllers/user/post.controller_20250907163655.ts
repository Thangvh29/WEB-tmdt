...
export const getUserPosts = [
  ...
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      const authorId = (user as any)._id?.toString();

      const posts = await Post.find({ author: new Types.ObjectId(String(authorId)) })
        .sort({ createdAt: -1 })
        .skip(skip)
        .limit(limit)
        .populate('author', 'name avatar role')
      ...
      const total = await Post.countDocuments({ author: new Types.ObjectId(String(authorId)) }).exec();
...
export const getPostDetail = [
  ...
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      Post.findByIdAndUpdate(id, { $inc: { views: 1 } }).exec().catch(() => { /* ignore */ });
  ...
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      const posts = await Post.find({
        ...
        .populate("author", "name avatar role") // ✅ cần populate để lấy avatar
        .sort({ createdAt: -1 })
        .skip(skip)
        .limit(limit)
        .lean()
      ...
      // Map avatar thành URL tuyệt đối, giữ likedByMe và likeCount
      const enriched = userPosts.map((p: any) => ({
        ...p,
        author: {
          ...p.author,
          avatar: buildFileURL(req, p.author?.avatar, 'avatar'), // ✅ fix avatar
        },
        likedByMe: userId
          ? (p.likes || []).some((id: any) => String(id) === userId)
          : false,
