...
function toAbsoluteUrl(req: Request, filePath: string | null | undefined) {
  ...
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      ...
      const variantSummary = variants.map((v: any) => ({
        ...
        stock: v.stock,
        images: Array.isArray(v.images) ? v.images.map((img: string) => toAbsoluteUrl(req, img)) : [],
      ...
      return res.json({
        product: {
          ...
          brand: product.brand,
          type: product.type,
          images: Array.isArray(product.images) ? product.images.map((img: string) => toAbsoluteUrl(req, img)) : [],
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const mapped = reviews.map(r => ({
        ...r,
        author: r.author
          ? { ...r.author, avatar: toAbsoluteUrl(req, (r.author as any).avatar) }
          : null,
      }));

      const total = await Comment.countDocuments({ product: productId, isDeleted: { $ne: true }, rating: { $ne: null } }).exec();

      return res.json({ reviews: mapped, page, limit, total });
    } catch (err) {
