 ...
 * ================================
 */
export const listOldProducts = async (req, res, next) => {
  try {
    const { brand, type, minPrice, maxPrice, page = 1, limit = 12 } = req.query;

    const filter = { isNewProduct: false }; // chỉ lấy sản phẩm cũ
