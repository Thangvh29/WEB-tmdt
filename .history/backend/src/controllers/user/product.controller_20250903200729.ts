...
import { toAbsoluteUrl, mapImages } from "../../urlHelper.jsutils/urlHelper";
...
export const listProducts = [
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      // parse isNew safely (req.query.* can be string | ParsedQs | string[]...)
      const isNewRaw = req.query.isNew;
      const isNew =
        isNewRaw === undefined ? undefined : (String(isNewRaw).toLowerCase() === 'true');
      ...
      if (q) {
        // text search - make sure you have text index on product fields
  ...
  return {
    ...
    brand: p.brand,
    type: p.type,
    image: toAbsoluteUrl(req, Array.isArray(p.images) && p.images.length > 0 ? p.images[0] : null), // 🔑 fix ở đây
