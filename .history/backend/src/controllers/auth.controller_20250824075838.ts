...
// Đăng nhập user
// Đăng nhập user (chuẩn Bearer + cookie)
export const login = [
  loginValidation,
  async (req: Request, res: Response, next: NextFunction) => {
    const errors = validationResult(req);
    ...
    try {
      ...
      res
        ...
        .header('Authorization', 'Bearer ' + token) // thêm header để client/ Postman dễ test
        .json({
          message: 'Đăng nhập thành công',
          token, // frontend lưu vào localStorage/sessionStorage
          user: {
            id: user._id,
            name: user.name,
            email: user.email,
