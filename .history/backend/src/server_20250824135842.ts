...
(async () => {
  ...
  try {
    ...
    try {
      ...
      const createChatRouter = (chatMod as any).createChatRouter;
    ...
    try {
      ...
      const chatRouter = (chatMod as any).default;
      if (chatRouter && typeof chatRouter === 'object' && 'use' in chatRouter) {
    ...
    // ---- Port/Host/BACKEND_URL setup (fix) ----
    const port = Number(process.env.PORT ?? config.PORT ?? 5000); // default 5000
    const host = process.env.HOST ?? config.HOST ?? '0.0.0.0';

    server.listen(port, host, () => {
      // Prefer config.BACKEND_URL if provided; otherwise compute from host/port
      ...
      // Expose to the whole app (controllers/middlewares can read)
      try {
        (app as any).locals.BACKEND_URL = backendURL;
        (app as any).set?.('BACKEND_URL', backendURL);
      } catch {}

      // Also make sure env has it (fallback for modules that import from env)
