  ...
  try {
    ...
    try {
      ...
      if (typeof createUserChatRouter === 'function' && io) {
        const chatRouter = createUserChatRouter(io);
        if (chatRouter && typeof chatRouter === 'object' && 'use' in chatRouter) {
          ...
          console.warn('User chat router is not a valid Express router; skipping.');
        }
      } else {
        console.warn('createUserChatRouter is not a function or io not available; skipping.');
      }
    } catch (err) {
      console.warn('Could not import/mount user chat routes:', util.inspect(err, { depth: 3 }));
    }
    ...
    try {
      ...
      if (typeof createAdminChatRouter === 'function' && io) {
        const chatRouter = createAdminChatRouter(io);
        if (chatRouter && typeof chatRouter === 'object' && 'use' in chatRouter) {
          (app as any).use('/api/admin/chat', chatRouter);
          console.log('✅ Mounted admin chat router at /api/admin/chat');
        } else {
          console.warn('Admin chat router is not a valid Express router; skipping.');
        }
      } else {
        console.warn('createAdminChatRouter is not a function or io not available; skipping.');
      }
    } catch (err) {
      console.warn('Could not import/mount admin chat routes:', util.inspect(err, { depth: 3 }));
    }
