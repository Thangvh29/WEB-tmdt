...
export interface IVariant {
  version: string;   // phiên bản (ví dụ: 16GB RAM, 512GB SSD)
  color: string;     // màu sắc
  price: number;     // giá bán
  stock: number;     // tồn kho
  images?: string[]; // ảnh riêng của biến thể
  createdAt?: Date;
  updatedAt?: Date;
}
...
export interface IProduct extends Document {
  ...
  // Methods
  getVariant(version: string, color: string): IVariant | null;
  getMinPrice(): number;
  getMaxPrice(): number;
...
ProductSchema.pre('save', function (this: HydratedDocument<IProduct>, next) {
  if (this.variants && this.variants.length > 0) {
    // cập nhật stock = tổng stock biến thể
    this.stock = this.variants.reduce((sum, v) => sum + (v.stock || 0), 0);
    // cập nhật giá = giá nhỏ nhất
...
// Methods
ProductSchema.methods.getVariant = function (this: IProduct, version: string, color: string) {
  if (!this.variants?.length) return null;
  return this.variants.find(v => v.version === version && v.color === color) || null;
};
...
ProductSchema.index({ 'variants.version': 1, 'variants.color': 1 });
