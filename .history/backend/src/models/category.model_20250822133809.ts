...
CategorySchema.pre('save', async function (this: ICategory, next) {
  try {
    ...
    if (!baseSlug) baseSlug = slugify(this.name || '', { lower: true, strict: true });
    let slug = baseSlug;
...
CategorySchema.statics.getTree = async function thisGetTree(this: CategoryModel) {
  // `this` is the model
  const cats = await this.find().lean().exec();

  const buildTree = (parentId: Types.ObjectId | null) => {
    return cats
      .filter((cat: any) =>
  ...
  return buildTree(null) as (ICategory & { children?: ICategory[] })[];
