...
CategorySchema.statics.getTree = async function (): Promise<ICategory[]> {
  const categories = await this.find().populate('children').lean() as ICategory[];
  const buildTree = (parentId: Types.ObjectId | null): ICategory[] => {
    return categories
      .filter((cat: ICategory) => (
        parentId ? (cat.parent?._id && cat.parent._id.equals(parentId)) : (!cat.parent)
      ))
