...
const Login = () => {
  ...
  try {
    ...
    if (response.ok) {
      // Backend trả về user + token (qua cookie)
      login({ identifier: form.identifier, role: data.user.role });

      // Nếu là admin thì điều hướng sang dashboard
      if (data.user.role === "admin") {
        navigate("/admin"); // hoặc "/admin/statistics" tuỳ bạn set route
      } else {
        navigate("/"); // user thì về Home
      }
    } else {
