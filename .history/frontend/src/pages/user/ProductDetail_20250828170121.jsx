...
const ProductDetail = () => {
  ...
  // gọi API sản phẩm liên quan + review
  useEffect(() => {
    api.get(`/api/products/${id}/related`).then(({ data }) => {
      setRelated(data.related || []);
    });
    api.get(`/api/products/${id}/reviews`, { params: { page: 1, limit: 6 } })
      .then(({ data }) =>
        setReviews({
  ...
  // gọi API check biến thể
  const onCheckVariant = async () => {
    if (!attrs.length) return;
    setChecking(true);
    try {
      const { data } = await api.post(`/api/products/${id}/check-variant`, { attrs });
                    ...
                    {values.map((val) => (
                      <button
                        ...
                        className={`px-3 py-1 border rounded ${
                          selected === val ? "bg-black text-white" : ""
                        }`}
                      >
                        {val}
                      </button>
