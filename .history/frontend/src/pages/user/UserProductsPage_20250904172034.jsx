...
const UserProductsPage = () => {
  ...
  const fetchProducts = useCallback(async () => {
    setLoading(true);
    try {
      let endpoint = "/user/products"; // mặc định
      if (filters.isNew === true) {
        endpoint = "/user/products/new";
      } else if (filters.isNew === false) {
        endpoint = "/user/products/old";
      }

      // copy filters nhưng bỏ isNew vì backend không hiểu
      const { isNew, ...query } = filters;
      const params = {};
