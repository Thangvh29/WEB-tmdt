...
import api from "../../services/axios"; // axios instance config sẵn
import ProductCard from "../../components/user/ProductCard";
import ProductFilters from "../../components/user/";
...
const Products = () => {
  ...
  useEffect(() => {
    ...
    api
      ...
      .catch(() =>
        mounted && setData({ products: [], total: 0, page: 1, limit: params.limit })
      )
  ...
  const totalPages = Math.max(
    1,
    Math.ceil((data.total || 0) / (data.limit || params.limit))
  );
        ...
        <>
          ...
          <div className="flex items-center justify-center gap-2 mt-6">
            <button
              ...
              onClick={() =>
                setParams((s) => ({ ...s, page: Math.max(1, s.page - 1) }))
              }
            ...
            <button
              ...
              onClick={() =>
                setParams((s) => ({ ...s, page: Math.min(totalPages, s.page + 1) }))
              }
              className="px-3 py-2 border rounded disabled:opacity-50"
            >
              Sau
