...
import { getCart, updateCartItem, removeFromCart, checkout } from "../../services/cart";
...
const Cart = () => {
  ...
  const handleQtyChange = async (item, qty) => {
    if (qty < 1) return;
    await updateCartItem(item.product, { variant: item.variant, quantity: qty });
    loadCart();
  };

  const handleRemove = async (item) => {
    await removeFromCart(item.product, { variant: item.variant });
    loadCart();
  };
