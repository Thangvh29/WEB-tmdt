...
import '../../assets/style/user-messages.css'; // Giả sử CSS cho trang tin nhắn

const socket = io('http://localhost:5000'); // Thay bằng backend URL
...
const UserMessagesPage = () => {
  ...
  const fetchConversations = async () => {
    setLoading(true);
    try {
      // ✅ sửa endpoint: gọi /api/user/chat
      const { data } = await api.get('/api/user/chat');
      // Sort: ghim admin ở đầu
