...
const ProductFilters = ({ onChange, initial = {} }) => {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const { data } = await mod.fetchFilters();
  ...
  return (
    <div className="grid gap-3 md:grid-cols-6 sm:grid-cols-3 grid-cols-2 mb-4">
      ...
      />
      <select value={form.isNew} onChange={(e) => update("isNew", e.target.value)} className="border rounded px-3 py-2">
        <option value="">Mới / Cũ</option>
        <option value="true">Hàng mới</option>
        <option value="false">Hàng cũ</option>
      </select>
      <select value={form.type} onChange={(e) => update("type", e.target.value)} className="border rounded px-3 py-2">
        <option value="">Loại</option>
        {types.map((t) => (
          <option key={t.type} value={t.type}>{t.type} ({t.count})</option>
        ))}
      </select>
      <select value={form.brand} onChange={(e) => update("brand", e.target.value)} className="border rounded px-3 py-2">
      ...
      />
      <select value={form.sort} onChange={(e) => update("sort", e.target.value)} className="border rounded px-3 py-2">
