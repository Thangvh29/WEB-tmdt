...
const handleSubmitComment = async () => {
  ...
  try {
    const { data } = await api.post(`/user/posts/${postId}/comments`, {
      content: newComment,
      // bỏ parent đi, không cần gửi null
    });
    ...
    <div className="comment-section">
      ...
      <div className="flex items-center gap-2 mb-4">
        ...
        <button
          ...
          className="ml-2 px-3 py-1 text-sm bg-blue-500 text-white rounded-full hover:bg-blue-600 transition"
        >
          Đăng
        </button>
      </div>
          ...
          <div key={comment._id} className="flex items-start gap-2">
            ...
            <div className="flex-1">
              ...
              {comment.replies?.map((reply) => (
                <div
                  key={reply._id}
                  className="flex items-start gap-2 mt-2 ml-10"
                >
                  <img
