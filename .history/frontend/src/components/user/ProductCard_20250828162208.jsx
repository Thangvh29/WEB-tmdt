...
const ProductCard = ({ p }) => {
  ...
  return (
    <Link to={`/user/products/${p._id}`} className="block border rounded-xl p-3 hover:shadow-md relative">
      {!p.inStock && (
      ...
      <img
        ...
        loading="lazy"
      />
      <div className="text-sm text-gray-500 capitalize">{p.brand} • {p.type}</div>
      <div className="font-medium line-clamp-2 leading-snug">{p.name}</div>
      <div className="mt-2 text-red-600 font-semibold">{format(price)}</div>
