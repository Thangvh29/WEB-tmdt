...
const ProductForm = ({ productId = null, onSubmitSuccess }) => {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const { data } = await api.get('/api/categories'); // Gọi endpoint categories
        setCategories(data);
      } catch (err) {
        setMessage('❌ Lỗi tải danh mục, vui lòng thêm danh mục thủ công hoặc kiểm tra server');
        setCategories([]); // Đặt rỗng để tránh crash
      }
    };
    fetchCategories();
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.category || formData.category === '') {
      setMessage('❌ Vui lòng chọn danh mục trước khi lưu');
      setLoading(false);
      return;
    }
    try {
