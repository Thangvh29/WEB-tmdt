...
const normalizeFormData = (formData) => {
  return {
    ...formData,
    // BE mong chờ specifications thay vì specs
    specifications: formData.specs.map((s) => ({
      key: s.key,
      value: s.value,
    })),

    // Commitments giữ nguyên
    commitments: formData.commitments,

    // BE mong chờ sellPrice thay vì variants
    sellPrice: formData.variants.map((v) => ({
      _id: v._id,
      sku: v.sku,
      ...
      images: v.images || [],
      version:
        v.attributes?.find((a) => a.name === "version")?.value || "",
      color: v.attributes?.find((a) => a.name === "color")?.value || "",
      isDefault: v.isDefault || false,
    })),

    // Images vẫn giữ nguyên
